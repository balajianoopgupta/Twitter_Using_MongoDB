...
exports.checkLogin = function(req, res) {
	...
	loginCollection.findOne({username : username, password : password}, function(err, user) 
	{
		if (user) 
		{
			...
			console.log("checklogin" + req.session.username+ " is in the session");
...
exports.calulateValues = function(req, res) 
{
	res.header('Cache-Control','no-cache, private, no-store, must-revalidate, max-stale=0, post-check=0, pre-check=0');
...
{
	console.log("Inside the addTweet");
	res.header('Cache-Control','no-cache, private, no-store, must-revalidate, max-stale=0, post-check=0, pre-check=0');
	var tDate = new Date();
...
//db.tweets.update({"username":"balaji@gmail.com"},{$push:{tweets:{$each:[{"tweet":"Balaji is now on #twitter","date":"2016-04-15T23:35:20.714Z"}],$position:0}}})
	...
	if (hashtags.length > 0) {
		for (var item = 0; item < hashtags.length; item++) {
			getInfo = "INSERT INTO `twitter`.`tweets` (`username`, `tweet`, `hashtag`) VALUES ('"
					+ ses.user
					+ "', '"
					+ req.param("tweetval")
					+ "', '"
		...
		getInfo = "INSERT INTO `twitter`.`tweets` (`username`, `tweet`) VALUES ('"
				+ ses.user + "', '" + req.param("tweetval") + "')";
		console.log("My Query is:" + getInfo);
		mysql.fetchData(function(err, results) {
			console.log(results);
			if (err) {
...
/*	
	...
	var userTweets = "SELECT concat(fname,' ',lname) usrname, usr.twitterhandle, TW.TWEET "
			+ "FROM"
			+ "(SELECT tweet TWEET from tweets where username='"
			+ ses.user
			+ "')TW,"
			+ " users usr where username='"
			+ ses.user
	...
	mysql.fetchData(function(err, results) 
	{
		...
		else 
		{
			...
			for (var i = 0; i < results.length; i++) 
			{
				...
				{
					var newTweet = (results[i].tweet).replace('#'
							+ arrayWithHashes[j], '<a href=\"#\">' + '#'
							+ arrayWithHashes[j] + '</a>');
...
/*	
	...
	var userFollowing = "SELECT concat(fname,' ',lname) usrname, usr.twitterhandle,FW.FOLLOW"
			+ "	FROM "
			+ "   (SELECT follows FOLLOW  from followers where uname='"
			+ ses.user + "')FW," + "    users usr where username=FW.FOLLOW;";
...
/*	
	...
	var userFollowedBy = "SELECT concat(fname,' ',lname) usrname, usr.twitterhandle,FB.FOLLOWEDBY "
			+ "	FROM "
			+ "  (SELECT uname FOLLOWEDBY  from followers where follows='"
			+ ses.user
			+ "')FB, "
...
/*
	...
	var getInfo = "select concat(u.fname,' ',u.lname)usrname, twitterhandle "
			...
			+ " from followers " + " where uname='" + ses.user
			+ "' and follows=u.username) and username!='" + ses.user + "'";
...
/*
	...
	var getInfo = "INSERT INTO `twitter`.`followers` (`uname`, `follows`) SELECT '"
			+ ses.user
			+ "', username FROM users u where twitterhandle='"
			+ req.param("thandler") + "';";
...
/*
	var getInfo = "SELECT concat(fname,' ',lname) usrname,usr.twitterhandle,usr.phone,usr.dob,usr.location, F.FOLLOWERS, T.FOLLOWING, TW.TWEETS"
			+ "	FROM "
			+ "	(SELECT count(*) FOLLOWERS 	from followers	where follows='"
			+ ses.user
			+ "')F,"
			+ "	(SELECT count(*) FOLLOWING 	from followers	where uname='"
			+ ses.user
			+ "')T,"
			+ "	(SELECT count(*) TWEETS	from tweets	where username='"
			+ ses.user
			+ "')TW,"
			+ "	users usr where username='"
			+ ses.user
...
exports.addNewUser = function(req, res) 
{
	...
	var connStr = 'mongodb://localhost:27017/login';
	...
	testUser.save(function(err, user) {
		console.log("Inside the save fundtion");
		if (err)
			throw err;
		/*
		 * if (user) { // This way subsequent requests will know the
		 * user is logged in. req.session.username = user.username;
		 * console.log(req.session.username + " is in the session");
		 * json_responses = {"statusCode" : 200};
		 * res.send(json_responses); } else { console.log("Insertion of
		 * new user failed. returned false"); json_responses =
		 * {"statusCode" : 401}; res.send(json_responses); }
		 */
	});
	/*
	 * if (err) throw err;
	 *  // fetch user and test password verification User.findOne({
	 * username: 'jmar777' }, function(err, user) { if (err) throw err;
	 *  // test a matching password user.comparePassword('Password123',
	 * function(err, isMatch) { if (err) throw err;
	 * console.log('Password123:', isMatch); // -> Password123: true });
	 *  // test a failing password user.comparePassword('123Password',
	 * function(err, isMatch) { if (err) throw err;
